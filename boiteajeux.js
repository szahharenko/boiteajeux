...
var createForm = function(game, code, name) {
 ...
 if(cols.length == 5) {
  ...
  cols[0].innerHTML = '<form id="form'+index+'" action="gestion.php" method="post" onsubmit="return validationUnique()"><input type="hidden" name="pAction" value="rejoindre"><input type="hidden" class="clInv" name="inv"><input type="hidden" name="id" value="'+code+'-'+id+'"><input type="image" src="./img/ico_join.png" alt="JOIN" class="clIcone" rel="popover" data-content="JOIN" onclick="return confirmJoin(' + index + ',\''+code+'\',\''+name+'\')" data-original-title=""></form>';
 }
